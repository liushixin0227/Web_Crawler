...
def add_filehandler(level, fmt, filename , mode, backup_count, limit, when, filename_fmt = 1):
    ...
    #time suffix to log run
    if filename_fmt == 1:
        filename = filename+"_"+time.strftime("%Y-%m-%d", time.localtime())
...
#set_logger时传入filename_fmt，且值不为1时，将不会在log文件名后面带上日期
def set_logger(filename = None, mode = 'a', level='CRITICAL:DEBUG',
               #fmt = '[%(levelname)s] %(asctime)s %(message)s',
               fmt = '%(asctime)s %(levelname)6s  %(filename)-10s  %(lineno)4d:%(funcName)16s|| %(message)s',
               backup_count = 10, limit = 1024*1024*200, when = None, filename_fmt = 1):
    ...
    add_filehandler(f_level, fmt, filename, mode, backup_count, limit, when, filename_fmt)
